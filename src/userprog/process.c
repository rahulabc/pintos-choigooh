...
get_thread(tid_t tid)
{
	...
	{
		struct thread *cur = list_entry(e, struct thread, table_elem);
		if(cur->tid==tid)
			return cur;
	}
	return NULL;
...
tid_t
process_execute (const char *file_cmd) 
{
  struct thread *cur = thread_current();
  struct thread *child;
  tid_t tid;

  char *temp = malloc(sizeof(char)*(strlen(file_cmd)));
	char *file_name;
	char *fn_copy;
	char *saveptr;
  ...
  list_push_back(&cur->child_list, &child->process_elem);
  child->parent = cur;
...
process_wait (tid_t child_tid UNUSED) 
{
    ...
    // Given tid, looking for the child
    ...
    // create a Wthread to enqueue process to the wait list
	struct Wthread *w = malloc(sizeof(struct Wthread));
	w->t = cur;
	w->child = t->tid;
	w->is_zombie = false;
	w->status = 0;
	...
	while(true)
	{
		...
		list_push_back(&wait_list, &w->elem);
		thread_block();
...
// preprocessing for process exit
void process__exit(int status)
{
	struct thread *cur = thread_current ();
	...
	while(list_size(&cur->open_file_list) > 0)
	{
		...
		struct file *f = list_entry(e, struct file, list_elem); 
		file_close(f);
		break;
	}

    // if it has children, get initial process to adopt the children
	struct list *child_list = &cur->child_list;
	struct list_elem *e;
	for(e=list_begin(child_list); e!=list_end(child_list); e=list_next(e))
	{
		struct thread *t = list_entry(e, struct thread, process_elem);
		struct thread *init_thread = get_thread(1);
		t->parent = init_thread;
		list_remove(&t->process_elem);
		list_push_back(&init_thread->child_list, &t->process_elem);
	}
	...
	list_remove(&cur->table_elem);

    // if someone is waiting for the process, wake up
	for(e=list_begin(&wait_list); e!=list_end(&wait_list); e=list_next(e))
	{
		struct Wthread *w = list_entry(e, struct Wthread, elem);
		if(w->child==cur->tid)
		{
            // deliver the status and wake up
			w->status = status;
			w->is_zombie = true;
			list_remove(&w->elem);
			thread_unblock(w->t);
